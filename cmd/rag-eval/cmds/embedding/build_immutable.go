...
type buildImmutableSettings struct {
	...
	RequestTimeoutSeconds int    `glazed:"request-timeout-seconds"`
}
...
func (c *buildImmutableCommand) RunIntoGlazeProcessor(ctx context.Context, vals *values.Values, processor middlewares.Processor) error {
	...
	if s.RequestTimeoutSeconds < 0 {
		return errors.New("request timeout seconds must not be negative")
	}
	if s.RequestTimeoutSeconds > 0 {
		var cancel context.CancelFunc
		ctx, cancel = context.WithTimeout(ctx, time.Duration(s.RequestTimeoutSeconds)*time.Second)
		defer cancel()
	}
