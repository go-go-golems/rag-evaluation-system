...
function site() {
	...
	function calendarEventsForPoll(poll, options, participants, voteMap, tally) {
		...
		return options
			.map((opt) => {
				...
				const availability = [];
				if (yesNames.length) availability.push(`yes: ${yesNames.join(", ")}`);
				if (maybeNames.length) availability.push(`maybe: ${maybeNames.join(", ")}`);
				if (noNames.length) availability.push(`no: ${noNames.join(", ")}`);
				return {
					id: `slot-${opt.id}`,
					title: opt.label,
					label: availability.length
						? `${opt.label} — ${availability.join("; ")}`
						: `${opt.label} — no responses yet`,
	...
	function calendarHourRange(events) {
		if (events.length === 0) return { start: 8, end: 18 };
		const hours = events.map((event) => Number(event.hour || 9));
		const minHour = Math.max(0, Math.min.apply(null, hours) - 1);
		const maxHour = Math.min(24, Math.max.apply(null, hours) + 2);
	...
	function pollPage(pollId) {
		...
		const calendarEvents = calendarEventsForPoll(poll, options, participants, voteMap, tally);
		const calendarHours = calendarHourRange(calendarEvents);
		const calendarMonth = calendarEvents.length
			? widget.time.month(
					{ monthISO: calendarEvents[0].dayISO.slice(0, 7), events: calendarEvents },
					(m) => m.selected(calendarEvents[0].dayISO),
				)
			: null;
		const calendarWeek = calendarEvents.length
			? widget.time.week(calendarEvents, (w) =>
					w
						.range(widget.time.range.week(calendarEvents[0].dayISO))
			...
			widget.page(poll.title, (p) => {
				applyPageMeta(p, "poll", "index")
					...
					.section("Calendar view", (s) =>
						s
							.caption(
								"The same offered slots are rendered as calendar widgets. Each week block lists who answered yes, maybe, or no for that time.",
							)
							.view(
								calendarEvents.length === 0
									...
									: widget.ui.stack({ gap: "md" }, calendarMonth, calendarWeek),
							),
					)
