	...
	render: () => (
		<Stack gap="md">
			<Section title="Common states">
				<FormRow
					label="Default"
					control={<TextInput defaultValue="sample-pi-session.jsonl" />}
					hint="Normal editable text input."
				/>
				<FormRow
					label="Placeholder"
					control={<TextInput placeholder="Enter display name" />}
					hint="Empty input with placeholder copy."
				/>
				<FormRow
					label="Disabled"
					control={<TextInput defaultValue="Locked by system" disabled />}
					hint="Native disabled state; not submitted by forms."
				/>
				<FormRow
					label="Read-only"
					control={<TextInput defaultValue="session-9a7c" readOnly />}
					hint="Readable but intentionally not editable."
				/>
				<FormRow
					label="Invalid"
				...
				<FormRow
					label="Required"
					control={<TextInput required placeholder="Required field" />}
					hint="Uses native required validation."
				/>
			</Section>
		...
		<Panel title="Settings form sketch" density="condensed">
			<Stack gap="sm">
				<FormRow
					...
					hint="Shown next to uploads from this browser."
				/>
				<FormRow
					label="Max rows"
					control={<TextInput type="number" min={1} max={500} defaultValue={100} />}
					hint="Numeric input in the same row anatomy."
				/>
				<FormRow
					label="Session id"
					control={<TextInput defaultValue="sess_8f2c…" readOnly />}
					hint="Read-only system value."
				/>
			</Stack>
		</Panel>
	),
