...
type PaletteControlsArgs = Omit<ContextLegendProps, "items" | "styles"> & {
...
export const PaletteControls: StoryObj<PaletteControlsArgs> = {
	...
	render: ({ palette, ...args }) => {
		...
		return (
			<Panel title={`legend · ${palette}`}>
				<ContextLegend {...args} items={styleSet.legend} styles={styleSet.styles} />
			</Panel>
		);
	},
};
...
export const DefaultStyleSet: Story = {
	render: () => (
		<Panel title="default context-window legend">
			<ContextLegend
		...
		<Stack gap="md">
			{(["xs", "sm", "md", "lg"] as const).map((size) => (
				<Panel key={size} title={`${size} legend`}>
					<ContextLegend
						items={contextDefaultStyleSet.legend.slice(0, 8)}
						styles={contextDefaultStyleSet.styles}
						size={size}
						selectedId="result"
					/>
