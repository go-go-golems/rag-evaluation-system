...
export const contextUploadDropAreaWidget = defineWidget<ContextUploadDropAreaWidgetProps>({
  ...
  render: (props, _children, ctx) => (
    <ContextUploadDropArea
      ...
      active={props.active}
      onFilesSelected={props.onFilesSelectedAction
        ? (files) => { void runFileSelectionAction(props.onFilesSelectedAction!, files, ctx); }
        : undefined}
    />
  ),
});

async function runFileSelectionAction(action: ActionSpec, files: File[], ctx: RenderContext): Promise<void> {
