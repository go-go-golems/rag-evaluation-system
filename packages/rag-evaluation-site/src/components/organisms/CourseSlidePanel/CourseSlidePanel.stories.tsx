...
export const SlideDeckSamples: Story = { render: () => <Stack gap="md">{contextSlides.slice(0,3).map((s, i) => <CourseSlidePanel key={s.id} slide={s} snapshot={contextWindowSnapshots.find((item) => item.id === s.snapshotId)!} index={i} total={contextSlides.length} />)}</Stack> };
