...
const irCourseStudioNavSections = [
	{
		...
		label: "Present",
		items: [
			{ id: "course", label: "Course", icon: component("ContextStudioNavIcon", { id: "course" }) },
			{ id: "slides", label: "Slides", icon: component("ContextStudioNavIcon", { id: "slides" }) },
		],
	},
	{
		id: "analyze",
		...
		items: [
			{
				id: "visualize",
				label: "Visualize",
				icon: component("ContextStudioNavIcon", { id: "visualize" }),
			},
			{ id: "upload", label: "Upload", icon: component("ContextStudioNavIcon", { id: "upload" }) },
		],
	},
	{
		id: "review",
		...
		items: [
			{
				id: "transcript",
				label: "Transcript",
				icon: component("ContextStudioNavIcon", { id: "transcript" }),
			},
			{
				id: "comments",
				label: "Comments",
				icon: component("ContextStudioNavIcon", { id: "comments" }),
			},
		],
	},
	{
		...
		items: [
			{
				id: "handout",
				label: "Handout",
				icon: component("ContextStudioNavIcon", { id: "handout" }),
			},
		],
	},
];
...
export const PaletteControls: StoryObj<PaletteControlsArgs> = {
	...
	render: ({ palette, registry, onAction }) => (
		<WidgetRenderer
			registry={registry}
			onAction={onAction}
			node={courseSlideWithContextVisualNode(contextStyleSetForPalette(palette))}
		/>
	),
};
		...
		node: component("SlideShell", {
			...
			primary: component(
				"FigureBlock",
				{
					...
					legend: component("ContextLegend", {
						items: contextDefaultStyleSet.legend,
						styles: contextDefaultStyleSet.styles,
						size: "sm",
					}),
				},
				[component("ContextBudgetBar", { snapshot, styleSet: contextDefaultStyleSet })],
			),
			secondary: component("KeyPointList", {
				items: [
					{
						id: "one",
						title: "Direct nodes",
						text: "Use low-level nodes when the page needs custom structure.",
					},
					{
						id: "two",
						title: "Recipes later",
						text: "Use recipes when the product composition is stable.",
					},
					{
						id: "three",
						title: "React renderer",
						text: "All visuals still come from package components.",
					},
				],
			}),
...
export const CoursePlusHandoutSplitView: Story = {
	args: {
		node: component("SplitPane", {
			ratio: "balanced",
			divider: true,
			left: component("CourseSlidePanel", {
				slide,
				snapshot,
	...
	args: {
		node: component("SplitPane", {
			...
			left: component("DocumentListPanel", {
				...
				items: contextHandoutFixture.docs.map((doc) => ({
					id: doc.id,
					title: doc.title,
					format: doc.format,
					size: doc.size,
			...
			right: component("Stack", { gap: "sm" }, [
				component("DocumentPreviewToolbar", {
					...
					onDownloadAction: { kind: "event", event: "widget-ir:document-download" },
					rightSlot: component("AnnotationBadge", {
						visualStyle: contextDefaultStyleSet.styles.course,
						label: "selected",
					}),
				}),
