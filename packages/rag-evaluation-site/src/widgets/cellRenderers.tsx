...
export function renderCell(
	...
	switch (spec.kind) {
		...
		case "status": {
			...
			return (
				<StatusText status={status} icon={spec.icon}>
					{status}
				</StatusText>
			);
		}
		case "caption":
			return (
				<Caption tone={spec.tone}>{stringify(getPath(row, spec.field), spec.fallback)}</Caption>
			);
		case "template":
		...
		case "link": {
			...
			return (
				<a
					href={href}
					target={spec.target}
					rel={spec.target === "_blank" ? "noreferrer" : undefined}
				>
					{label}
				</a>
			);
...
export function rowKey(
	row: JsonObject,
	spec: string | { field: string } | { template: string },
): string {
