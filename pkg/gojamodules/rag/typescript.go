...
func TypeScriptModule() *spec.Module {
	return &spec.Module{Name: ModuleName, Description: "Typed fluent immutable RAG laboratory experiments.", RawDTS: []string{
		...
		"export interface RAGExportOptions { datasetSplit: string; }",
		"export interface RAGExecutionSpecificationV1 { schemaVersion: 'rag-retrieval-spec/v1'; name: string; dataset: { split: string }; representations: Array<{ name: string; kind: 'rawChunks' | 'summary' | 'questions' }>; retrieval: { channels: Array<{ name: string; backend: 'bm25' | 'vector'; representation: string; topK: number; filter?: Record<string, any> }>; filter?: Record<string, any>; fusion?: { kind: 'rrf'; rankConstant: number; weights?: Record<string, number> }; reranking?: { kind: string; model: string; candidateCount: number; results: number }; collapse: 'none' | 'document' | 'parentChunk'; results: number }; metrics: { relevanceAt: Exclude<RelevanceGradeName, '0_FAIL'>; recallAt?: number[]; precisionAt?: number[]; ndcgAt?: number[]; mrr: boolean }; tags?: Record<string, string>; }",
		"export interface Experiment { use(fragment: Fragment): this; corpus(ref: string | ArtifactRef): this; chunks(ref: string | ArtifactRef): this; bm25(ref: string | ArtifactRef): this; embeddings(ref: string | ArtifactRef): this; evaluation(ref: string | ArtifactRef): this; note(text: string): this; tag(name: string, value?: string): this; representations(configure: (builder: RepresentationsBuilder) => void): this; retrieval(configure: (builder: RetrievalBuilder) => void): this; metrics(configure: (builder: MetricsBuilder) => void): this; validate(lab?: Laboratory): ValidationReport; toSpec(): ExperimentSpecification; toJSON(): ExperimentSpecification; exportSpecification(options: RAGExportOptions): RAGExecutionSpecificationV1; }",
