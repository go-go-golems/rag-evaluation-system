...
func TypeScriptModule() *spec.Module {
	return &spec.Module{Name: ModuleName, Description: "Typed fluent immutable RAG laboratory experiments.", RawDTS: []string{
		...
		"export interface RetrievalBuilder { channel(name: string, configure: (channel: ChannelBuilder) => void): this; filter(configure: (filter: FilterBuilder) => void): this; fuse(configure: (fusion: FusionBuilder) => void): this; collapse(scope: 'none' | 'parentChunk' | 'document'): this; results(count: number): this; }",
		...
		"export interface OpenOptions { database: string; execution?: 'readOnly' | 'allowRuns'; }",
		"export interface Laboratory { validate(experiment: Experiment): ValidationReport; persist(experiment: Experiment): { id: string; reused: boolean; schemaVersion: string }; start(experiment: Experiment): { id: string; experimentSpecId: string; status: string }; close(): void; }",
