...
## O. geppetto — `~/code/wesen/go-go-golems/geppetto`

### O1. `README.md` (root) — 🟢

- **What I was researching:** the geppetto runtime model and its JS module surface.
- **What I was looking for:** the registry-first/profile-first model, the building blocks list, the `require("geppetto")` mention.
- **Why I chose it:** root README; user flagged geppetto as interesting.
- **How I found it:** user asked to check geppetto; `ls` + `head README.md`.
- **What I found useful:** the building-blocks list (`pkg/turns`, `pkg/inference/engine`, `toolloop`, `middleware`, `session`, `pkg/profiles`, `pkg/js/modules/geppetto`), the explicit "native JavaScript bindings through Goja (`require(\"geppetto\")`)" statement, the registry source stack (`yaml`/SQLite file/`sqlite-dsn`).
- **What I didn't find useful:** the README focuses on the Go runtime model; the JS DSL API itself is not in the README.
- **What is out of date / what was wrong:** none observed.
- **What would need updating:** add a JS DSL section showing the `engine()`/`agent()`/`schema()` builder chains.

### O2. `pkg/js/modules/geppetto/module.go` — 🟢

- **What I was researching:** the module registration, the hidden-ref substrate, and the export surface.
- **What I was looking for:** `hiddenRefKey`, `mustSet`, the top-level exports.
- **Why I chose it:** the module entrypoint.
- **How I found it:** `find pkg/js -name '*.go'`.
- **What I found useful:** `ModuleName = "geppetto"`, `hiddenRefKey = "__geppetto_ref"` (same substrate as goja-bleve), the `Options` struct carrying `runtimeowner.RuntimeOwner` (ownership correctness), the exports list (`version`, `inferenceProfiles`, `engine`, `embeddings`, `agent`, `tool`, `toolRegistry`, `schema`).
- **What I didn't find useful:** nothing.
- **What is out of date / what was wrong:** none.
- **What would need updating:** none. Confirms geppetto reuses the goja-bleve typed-ref pattern.

### O3. `api_engine_builder.go`, `api_schema_builders.go`, `api_agent.go` — 🟢

- **What I was researching:** the fluent builder implementations.
- **What I was looking for:** the chain shape, the clone-vs-mutate question, terminal shape.
- **Why I chose it:** the builder implementations.
- **How I found it:** from O2.
- **What I found useful:** **clone-on-each-step** immutable builder variant (`cloneFor(m)`, returns new wrapper per method) — a notable difference from goja-bleve's same-object mutation; `.inference(settings).build()` chain; `schema.string()/object()/enum()` + `.description`/`.property` sub-builder; `agentBuilderRef` with name/base/settings/middlewares.
- **What I didn't find useful:** nothing.
- **What is out of date / what was wrong:** validation is mixed (`panic(NewTypeError)` for misuse) — not wrong, but a divergence from the `(value, error)` rule.
- **What would need updating:** the playbook should pick clone-vs-mutate as a rule; geppetto's clone is the safer default.

### O4. `dts_parity_test.go` + `pkg/doc/types/geppetto.d.ts` — 🟢 (key find)

- **What I was researching:** the compile-time-type discipline.
- **What I was looking for:** whether the generated `.d.ts` is enforced against the runtime.
- **Why I chose it:** the file name implied a parity test.
- **How I found it:** `ls pkg/js/modules/geppetto/`.
- **What I found useful:** `TestGeneratedDTSMatchesRuntimeExportSurface` parses `geppetto.d.ts` and asserts top-level + grouped namespaces (`consts`, `inferenceProfiles`, `schema`, `turnStores`) match the runtime `require("geppetto")` surface. The `.d.ts` is generated by `cmd/gen-meta from geppetto_codegen.yaml`. **This is the compile-time-type enforcement discipline the playbook wants, with a test.**
- **What I didn't find useful:** nothing.
- **What is out of date / what was wrong:** none.
- **What would need updating:** none. Should be cited as the canonical type-parity testing pattern.

---

## P. discord-bot — `~/code/wesen/go-go-golems/discord-bot`

### P1. `README.md` (root) — 🟢

- **What I was researching:** the discord-bot runtime model and JS API.
- **What I was looking for:** `require("discord")`, the available modules, the one-bot-per-process model.
- **Why I chose it:** root README; user flagged discord-bot.
- **How I found it:** user asked to check discord-bot.
- **What I found useful:** "Go owns the Discord gateway/session; JS owns behavior through `require(\"discord\")`", one selected JS bot per process, install methods, the module list (`discord`, `timer`, `database`, `ui`).
- **What I didn't find useful:** nothing.
- **What is out of date / what was wrong:** none observed.
- **What would need updating:** none.

### P2. `pkg/doc/topics/discord-js-bot-api-reference.md` — 🟢 (key find)

- **What I was researching:** the `defineBot` and `require("ui")` DSLs.
- **What I was looking for:** the `ui` builder API and its typed-builder design rules.
- **Why I chose it:** the API reference doc.
- **How I found it:** `rg 'require("ui")' pkg/doc/`.
- **What I found useful:** the `defineBot` helper table (command/event/component/modal/autocomplete/subcommand/configure), the `ui` builder table (message/embed/card/button/select/form/row/pager/actions/confirm/ok/error/emptyResults/flow), the design rules ("returns typed builders, not plain JS objects"; "wrong-parent calls should fail loudly"; "raw JS objects are not accepted where builders are expected"; `.followUp()` vs update-in-place), the Goja-Proxy-trap surfacing description.
- **What I didn't find useful:** nothing.
- **What is out of date / what was wrong:** none.
- **What would need updating:** none. Strongest documentation of a Proxy-trap typed-builder DSL.

### P3. `pkg/framework/framework.go` — 🟢

- **What I was researching:** the embedding API for host apps.
- **What I was looking for:** the functional-options constructor and runtime module registrars.
- **Why I chose it:** the public framework entrypoint.
- **How I found it:** `ls pkg/framework/`.
- **What I found useful:** `New(opts ...Option)` with `WithCredentialsFromEnv`/`WithScript`/`WithSyncOnStart`/`WithRuntimeModuleRegistrars` — the glazed-style functional-options embedding path; `Config` + `Bot` + `Credentials` structs.
- **What I didn't find useful:** nothing.
- **What is out of date / what was wrong:** none.
- **What would need updating:** none.

### P4. `examples/discord-bots/ping/index.js` — 🟢

- **What I was researching:** a complete `defineBot` example.
- **What I was looking for:** the full registration DSL in practice (commands, components, modals, autocomplete, events).
- **Why I chose it:** the canonical example bot.
- **How I found it:** `find examples -name '*.js'`.
- **What I found useful:** the full `defineBot(({command, event, component, modal, autocomplete, configure}) => {...})` shape, `ctx.defer`/`ctx.edit`/`ctx.followUp`/`ctx.showModal`/`ctx.discord.channels.send` with `files`, autocomplete returning choices.
- **What I didn't find useful:** nothing.
- **What is out of date / what was wrong:** none.
- **What would need updating:** none.
...
## Q. researchctl — `~/workspaces/2026-06-30/benchmark-cpu-inference/researchctl`

### Q1. `README.md` (root) — 🟢 (key find)

- **What I was researching:** the researchctl project model and JS grammar.
- **What I was looking for:** the claim/evidence/decision graph and the `require("researchctl")` fluent API.
- **Why I chose it:** root README; the user pointed at the benchmark-cpu-inference workspace for researchctl/codesign.
- **How I found it:** user request.
- **What I found useful:** the graph entity list (goals/questions/hypotheses/workPackages/experiments/sources/evidence/decisions/reports/reviewRules/views), the YAML format, the **JS grammar with the lambda-configurator example** (`project("X").goal("G", g => g.id("GOAL-001").status("active"))`), the trusted-JS-file model, the CLI commands.
- **What I didn't find useful:** nothing.
- **What is out of date / what was wrong:** none.
- **What would need updating:** none. This is the clearest example of the lambda-configurator pattern the playbook wants.

### Q2. `pkg/gojamodules/researchctl/module.go` — 🟢

- **What I was researching:** the module exports and the `project()` builder.
- **What I was looking for:** the `project`/`fromSpec`/`validate` exports and the TS `ProjectBuilder` interface.
- **Why I chose it:** the module entrypoint.
- **How I found it:** `find pkg/gojamodules/researchctl`.
- **What I found useful:** exports (`project`, `fromSpec`, `validate`), `project(name)` returns a `ProjectBuilder`, `valueToSpec` duck-types `.toSpec()`, the embedded TS declaration with the precise `ProjectBuilder` interface (typed method signatures).
- **What I didn't find useful:** the lambda callback param is typed `any` in the TS interface — the one weak spot.
- **What is out of date / what was wrong:** none.
- **What would need updating:** narrow the `build?: (g: any) => any` callback param to a typed sub-builder interface (codesign already does this).

### Q3. `pkg/gojamodules/researchctl/builders.go` — 🟢

- **What I was researching:** the entity sub-builders.
- **What I was looking for:** the fluent methods on each sub-builder.
- **Why I chose it:** the builder implementations.
- **How I found it:** from Q2.
- **What I found useful:** the entity method lists (`id`/`description`/`priority`/`status`/`asks`/`tag`/`hypothesize`/`sources`/`testedBy`/`evidence`/`decision`/`confidence`/`reversalCondition`/`kind`/`owner`/`team`/`dependsOn`/`blocks`/`tests`), same-object chaining returning `*goja.Object`.
- **What I didn't find useful:** nothing.
- **What is out of date / what was wrong:** none.
- **What would need updating:** none.
...
## R. codesign — `~/workspaces/2026-06-30/benchmark-cpu-inference/researchctl/pkg/gojamodules/codesign`

### R1. `module.go` — 🟢 (key find)

- **What I was researching:** the codesign module registration and exports.
- **What I was looking for:** the `NativeModule` + `TypeScriptDeclarer` interface implementation and the export list.
- **Why I chose it:** the module entrypoint.
- **How I found it:** `ls pkg/gojamodules/codesign/`.
- **What I found useful:** `var _ modules.NativeModule = (*module)(nil)` and `var _ modules.TypeScriptDeclarer = (*module)(nil)` — **the canonical typed-module registration pattern**; `TypeScriptModule()` returns `ts.Module` with `RawDTS`; the export list (`runSpec`/`compareMetric`/`reduceValues`/`validateRun`/`toSpec`/`run`/`manifest`/`writeArtifacts`/`configHash`/`summarize`/`context`/`toYaml`/`loadYaml`/`writeYamlArtifact`/`registerSource`/`registerCallbackSource`); the `callbackDevices`/`callbackPolicies`/`callbackMetrics` maps for JS-lambda registration.
- **What I didn't find useful:** nothing.
- **What is out of date / what was wrong:** none.
- **What would need updating:** none.

### R2. `builders.go` — 🟢 (key find)

- **What I was researching:** the `runSpec` builder and sub-builders.
- **What I was looking for:** the fluent chain, the lambda configurators, and the `.use()` fragment composition.
- **Why I chose it:** the builder implementation.
- **How I found it:** from R1.
- **What I found useful:** `runSpec(name)` → `runSpecBuilder` with `.experiment/.backend/.tag/.meta/.policy/.policyCallback/.topology/.workload/.metrics/.use/.validate/.toSpec/.run`; `topology(fn)`/`workload(fn)`/`metrics(fn)` apply a lambda configurator via `applyBuilderCallback`; `.policyCallback(id, fn)` validates `goja.AssertFunction` and stores the JS callback; `set := func(name, fn) { m.mustSet(obj, name, fn) }` helper.
- **What I didn't find useful:** nothing.
- **What is out of date / what was wrong:** none.
- **What would need updating:** none. The strongest composable-grammar model.

### R3. `typescript.go` — 🟢 (key find)

- **What I was researching:** the TypeScript declarations.
- **What I was looking for:** whether the builder types are precise (not `any`).
- **Why I chose it:** the TS declaration source.
- **How I found it:** `ls pkg/gojamodules/codesign/`.
- **What I found useful:** precise `interface`s — `RunSpecBuilder`, `TopologyBuilder` (with typed `device`/`jsDevice`/`cpu`/`accelerator`/...), `MetricsBuilder`, `RunSpecLike = RunSpec | RunSpecBuilder | { toSpec(): RunSpec }` (typed union), `FragmentFn<T>` for reusable lambdas. **This is the compile-time-types target the playbook wants.**
- **What I didn't find useful:** nothing.
- **What is out of date / what was wrong:** none.
- **What would need updating:** none.
...
- **What I was researching:** runnable examples.
- **What I was looking for:** the fluent chains in test assertions.
- **Why I chose it:** tests are reliable API docs.
- **How I found it:** `ls pkg/gojamodules/codesign/`.
- **What I found useful:** `.policy("min_finish_time")`, `.policy("first_available")`, the `r => r.experiment("EXP-001").backend("cpu-sim").policy(...)` reuse pattern.
- **What I didn't find useful:** nothing.
- **What is out of date / what was wrong:** none.
- **What would need updating:** none.
...
| geppetto `README.md` + `pkg/js/modules/geppetto/*` | 🟢 | typed-ref + clone-on-each-step builders; add JS DSL section to README |
| geppetto `dts_parity_test.go` + `geppetto.d.ts` | 🟢 | **canonical type-parity test** — cite for compile-time-type enforcement |
| discord-bot `README.md` + `pkg/doc/topics/discord-js-bot-api-reference.md` | 🟢 | `defineBot` + Proxy-trap `ui` typed builders; strongest UI-DSL docs |
| discord-bot `pkg/framework/framework.go` | 🟢 | functional-options embedding API |
| researchctl `README.md` + `pkg/gojamodules/researchctl/*` | 🟢 | **lambda-configurator pattern** (`g => g.id(...)`); narrow the `any` callback param |
| codesign `module.go` + `builders.go` + `typescript.go` | 🟢 | **strongest model**: `.use()` fragments + lambda configurators + precise TS `interface`s + `TypeScriptDeclarer` |
