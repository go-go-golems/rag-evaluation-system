...
- The exact monetary ceiling is 639,000 microunits (USD $0.639) for the 60-request smoke matrix, derived from the pinned 16,384-token input reservation, pinned 8,192-token response cap, and published Umans Flash rates of $0.15/M input and $1.00/M output tokens. The operator rejected this reduced ceiling, so the real run remains blocked; concurrency approval alone is not cost approval.
