...
export const DocProcessingIdentityBar: React.FC<DocProcessingIdentityBarProps> = ({
	...
	if (identities.length === 0) {
		return <Caption className={styles.empty}>No preprocessing identities found</Caption>;
	}

	return (
		<Inline gap="xs" className={styles.root} data-rag-component="DocProcessingIdentityBar">
			<Caption tone="inherit" className={styles.label}>
				Preprocessing:
			</Caption>
			{identities.map((identity) => {
				...
				return (
					...
					>
						{identity.artifact_type}/{identity.prompt_version} ({identity.provider}/{identity.model}
						) [{identity.artifact_count}]
					</Button>
				);
			})}
		</Inline>
		...
		<Inline gap="xs" className={styles.root} data-rag-component="ChunkEnrichmentIdentityBar">
			...
			{identities.map((identity) => {
				...
				return (
					...
					>
						{identity.strategy_id}/{identity.prompt_version} ({identity.provider}/{identity.model})
						[{identity.enriched_count}]
					</Button>
				);
			})}
		</Inline>
