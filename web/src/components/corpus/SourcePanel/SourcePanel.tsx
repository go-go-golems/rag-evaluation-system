...
import { CorpusSourceSummary, DocumentProcessingCoverageItem } from "../../../services/api";
...
const SourceItem: React.FC<SourceItemProps> = ({ source, selected, onClick, preprocessing }) => {
	const pct =
		source.chunk_count > 0 ? Math.round((source.embedded_count / source.chunk_count) * 100) : 0;
	const prePct =
		preprocessing && preprocessing.document_count > 0
	...
	return (
		...
		>
			...
			{source.chunk_count > 0 && (
				<Caption
					className={selected ? styles.selectedMeta : undefined}
					tone={selected ? "inherit" : pct === 100 ? "success" : pct > 0 ? "warning" : "muted"}
				>
			...
			{preprocessing && (
				<Caption
					...
					tone={
						selected
							? "inherit"
							: preprocessing.missing_count === 0
								? "success"
								: preprocessing.artifact_count > 0
									? "warning"
									: "muted"
					}
				>
...
export const SourcePanel: React.FC<SourcePanelProps> = ({
	...
	return (
		<Panel
			...
			title="Sources"
			actions={<Caption>{sources.length}</Caption>}
			density="condensed"
		>
