...
export const EmbeddingsView: React.FC = () => {
	...
	useEffect(() => {
		...
		if (
			defaultTargetChunk &&
			(!chunkIDB || !strategyChunks.some((chunk) => chunk.id === chunkIDB))
		) {
		...
		<div className={styles.root}>
				...
				<div className={styles.controlsGrid}>
					<fieldset className={styles.fieldset}>
						...
						<Stack gap="xs">
							...
							<Caption>
								{selectedStrategy
									? `${selectedStrategy.type}: ${selectedStrategy.description || "no description"}`
									: "Create chunks before computing embeddings."}
							</Caption>
						</Stack>
					</fieldset>
					...
					<fieldset className={styles.fieldset}>
						...
						<MetadataGrid
							...
							items={[
								...
								{
									key: "Last compute",
									value: computeState.data
										? `${computeState.data.computed} computed, ${computeState.data.skipped_fresh} fresh`
										: "—",
								},
							]}
						/>
			...
			<DashboardGrid recipe="twoColumn">
					...
					<Stack gap="sm">
							...
							<FormRow
								...
								control={
									<TextInput
										...
										min={0}
										value={computeLimit}
										onChange={(event) => setComputeLimit(Number(event.target.value))}
									/>
								}
							/>
						...
						<div className={styles.actions}>
							<Button
								...
								disabled={!canCompute || computeState.isLoading}
							>
							...
							<MetadataGrid
								...
								items={[
									...
									{ key: "Skipped fresh", value: computeState.data.skipped_fresh },
									{
										...
										value: (
											<CodeText>
												{computeState.data.provider_type}/{computeState.data.model} (
												{computeState.data.dimensions})
											</CodeText>
										),
									},
								]}
						...
						{computeState.error ? (
							<ErrorCallout>{formatApiError(computeState.error)}</ErrorCallout>
						) : null}
					</Stack>
				</Panel>
				...
				<Panel title="Pairwise / Bounded Similarity">
					<Stack gap="sm">
						<Caption>
							Similarity reads stored vectors only. Select a document with chunks, then compare.
						</Caption>
						<FormRow
							label="Document"
							control={
							...
							hint={
								selectedDocument
									? `${selectedDocument.status} · ${selectedDocument.word_count} words · ${selectedDocument.id}`
						...
						<Stack gap="xs">
							<FormRow
								...
								control={
									<SelectInput
										value={chunkIDA}
										onChange={(event) => setChunkIDA(event.target.value)}
									>
							...
							<FormRow
								...
								control={
									<SelectInput
										value={chunkIDB}
										onChange={(event) => setChunkIDB(event.target.value)}
									>
						...
						<div className={styles.actions}>
							<Button
								...
								disabled={!canCompare || similarityState.isLoading}
							>
								{similarityState.isLoading ? "Comparing..." : "Compare Similarity"}
							</Button>
						</div>
						{similarityState.error ? (
							<ErrorCallout>{formatApiError(similarityState.error)}</ErrorCallout>
						) : null}
					</Stack>
				</Panel>
			</DashboardGrid>
			...
			{similarityState.data ? (
				<Panel title="Similarity Results" density="condensed">
					<Caption className={styles.resultIntro}>
						Source: {similarityState.data.source.chunk_id} · {similarityState.data.considered}{" "}
						candidates
					</Caption>
					<DataTable
						...
						columns={[
							{
								id: "score",
								header: "Score",
								cell: (match) => <CodeText>{match.score.toFixed(6)}</CodeText>,
							},
							{
								id: "chunk",
								header: "Chunk",
								cell: (match) => <CodeText>{match.chunk_id}</CodeText>,
							},
							{
								id: "document",
								header: "Document",
								cell: (match) => <CodeText>{match.document_id}</CodeText>,
							},
							{ id: "index", header: "Index", align: "end", cell: (match) => match.chunk_index },
							{
								id: "preview",
								header: "Preview",
								cell: (match) => <Caption tone="inherit">{match.text_preview}</Caption>,
							},
						]}
					/>
				</Panel>
