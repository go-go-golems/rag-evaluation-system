...
export function ResultInspectorPanel({
	...
	const nextChunk =
		chunkIndex >= 0 && chunkIndex < (docDetail?.chunks?.length ?? 0) - 1
			...
			? [
					{
						...
						value: (
							<Caption tone="warning">
								rank #{result.components.bm25.rank}, score {result.components.bm25.score.toFixed(4)}
							</Caption>
						),
					},
				]
			...
			? [
					{
						...
						value: (
							<Caption tone="accent">
								rank #{result.components.vector.rank}, score{" "}
								{result.components.vector.score.toFixed(4)}
							</Caption>
						),
					},
				]
	...
	const documentItems: MetadataGridItem[] = docDetail
		? [
				...
				...(docDetail.document.url
					? [{ key: "URL", value: <Caption tone="accent">{docDetail.document.url}</Caption> }]
					: []),
	...
	const metadataItems: MetadataGridItem[] = docDetail?.document.metadata
		? Object.entries(docDetail.document.metadata)
				.slice(0, 12)
	...
	return (
		<Panel
			...
			actions={
				<div className={styles.actions}>
					<Button size="compact" onClick={onOpenInCorpus}>
						Open in Corpus
					</Button>
		...
		>
			<TabList
				...
				ariaLabel="Result inspector tabs"
			/>
								...
								<>
									<MetadataGrid
										items={[
											{
												key: "Offsets",
												value: `${currentChunk.start_offset}–${currentChunk.end_offset}`,
											},
											{ key: "Tokens", value: currentChunk.token_count },
											{
												key: "Embedding",
												value: (
													<StatusText status={currentChunk.embedding?.present ? "done" : "error"}>
								...
								{prevChunk && (
									<div>
										...
										<div className={styles.neighborBlock}>
											{prevChunk.text.slice(0, 300)}
											{prevChunk.text.length > 300 ? "…" : ""}
										</div>
									</div>
								)}
								{nextChunk && (
									<div>
										...
										<div className={styles.neighborBlock}>
											{nextChunk.text.slice(0, 300)}
											{nextChunk.text.length > 300 ? "…" : ""}
										</div>
									</div>
								)}
							</section>
							...
							<>
								...
								{docDetail.chunks.length > 0 && (
									<section className={styles.section}>
										...
										<Caption>
											{docDetail.chunks.filter((c) => c.embedding?.present).length}/
											{docDetail.chunks.length} embedded · <Caption tone="accent">■</Caption> =
											current
										</Caption>
									</section>
								)}
