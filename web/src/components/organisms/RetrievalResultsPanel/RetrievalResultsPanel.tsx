			...
			? [
					{
						...
						header: "BM25",
						align: "end" as const,
						cell: (item: RetrievalResult) =>
							item.components?.bm25 ? (
								<Caption tone="warning">
									#{item.components.bm25.rank} ({item.components.bm25.score.toFixed(3)})
								</Caption>
							) : (
								<Caption>—</Caption>
							),
					},
					{
						...
						cell: (item: RetrievalResult) =>
							item.components?.vector ? (
								<Caption tone="accent">
									#{item.components.vector.rank} ({item.components.vector.score.toFixed(3)})
								</Caption>
							) : (
								<Caption>—</Caption>
							),
	...
	return (
		...
		>
			<ScrollRegion axis="y" style={{ height: "100%" }}>
				...
				{isLoading && <Caption className={styles.centeredEmpty}>Searching…</Caption>}
				{items.length > 0 && (
					<DataTable
						rows={items}
						columns={columns}
						...
						onRowSelect={(item) =>
							onSelectResult(selectedResult?.chunk_id === item.chunk_id ? null : item)
						}
					/>
				)}
			</ScrollRegion>
