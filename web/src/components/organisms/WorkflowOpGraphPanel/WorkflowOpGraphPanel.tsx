...
export function WorkflowOpGraphPanel({ groups, total, done }: WorkflowOpGraphPanelProps) {
	const fanOutOps = groups.filter(
		(group) => group.operation === "chunk_document" || group.operation === "preprocess_document",
	);
	const fanInOps = groups.filter(
		(group) => group.operation === "compute_embeddings" || group.operation === "build_bm25",
	);
	const otherOps = groups.filter(
		(group) =>
			!fanOutOps.some(
				(candidate) => candidate.operation === group.operation && candidate.status === group.status,
			) &&
			!fanInOps.some(
				(candidate) => candidate.operation === group.operation && candidate.status === group.status,
			),
	...
	const renderNode = (group: WorkflowOpGroup, includeCount = true) => (
		<span key={group.operation + "|" + group.status} className={styles.node}>
			<StatusText status={workflowStatusToRagStatus(group.status)} icon>
				{friendlyOpName(group.operation)}
	...
	return (
		<Panel title="Op Graph" density="condensed" data-rag-component="WorkflowOpGraphPanel">
			<div className={styles.graph}>
				...
				{fanInOps.length > 0 && (
					<div className={styles.fanOut}>{fanInOps.map((group) => renderNode(group, false))}</div>
				)}
			</div>
		</Panel>
	);
