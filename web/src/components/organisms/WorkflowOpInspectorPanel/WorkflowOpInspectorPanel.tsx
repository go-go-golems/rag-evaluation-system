...
export function WorkflowOpInspectorPanel({
	sample,
	retrying = false,
	onClose,
	onRetry,
	children,
}: WorkflowOpInspectorPanelProps) {
	const inputItems = Object.entries(sample.op.Input as Record<string, unknown>).map(
		([key, value]) => ({
			key,
	...
	return (
		<Panel
			...
			actions={
				<IconButton label="Close sampled op inspector" onClick={onClose}>
					✕
				</IconButton>
			}
			density="condensed"
		...
		>
			<MetadataGrid
				items={[
					{ key: "ID", value: sample.op.ID, copyValue: sample.op.ID },
					{
						key: "Status",
						value: (
							<StatusText status={workflowStatusToRagStatus(sample.status)} icon>
								{sample.status}
							</StatusText>
						),
					},
			...
			{canRetry && (
				<ErrorCallout className={styles.error}>
					...
					<Caption>
						Attempt {sample.op.RetryState.Attempt}/{sample.op.Retry.MaxAttempts}
					</Caption>
