...
export function WorkflowOpResultPanel({
	result,
	isLoading = false,
	error,
}: WorkflowOpResultPanelProps) {
	...
	if (error) {
		...
		return (
			<ErrorCallout className={styles.error}>Failed to load result: {String(error)}</ErrorCallout>
		);
	}
	...
	const artifactColumns: DataTableColumn<ArtifactRow>[] = [
		...
		{ id: "kind", header: "Kind", cell: (artifact) => artifact.Kind },
		{
			id: "contentType",
			header: "Content Type",
			cell: (artifact) => <Caption>{artifact.ContentType}</Caption>,
		},
	];
			...
			{result.Records && result.Records.length > 0 && (
				<section className={styles.section}>
					...
					<DataTable
						...
						getRowKey={(record) => `${record.Table}:${record.PK}`}
					/>
				</section>
			)}
			...
			{result.Artifacts && result.Artifacts.length > 0 && (
				<section className={styles.section}>
					...
					<DataTable
						...
						getRowKey={(artifact) => `${artifact.Name}:${artifact.Kind}`}
					/>
				</section>
			)}
			...
			{result.EmittedIDs && result.EmittedIDs.length > 0 && (
				<section className={styles.section}>
					...
					<div className={styles.emittedList}>
						{result.EmittedIDs.slice(0, 20).map((id) => (
							<div key={id}>{id}</div>
						))}
