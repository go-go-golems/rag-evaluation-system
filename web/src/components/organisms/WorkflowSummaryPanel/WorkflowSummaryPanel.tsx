...
function ProgressBar({
	...
	return (
		<div className={styles.progressBar} data-rag-component="WorkflowProgressBar">
			...
			{running > 0 && (
				<div
					className={`${styles.progressFill} ${styles.running}`}
					style={{ width: `${runPct}%`, left: `${pct}%` }}
				/>
			)}
			{failed > 0 && (
				<div
					className={`${styles.progressFill} ${styles.failed}`}
					style={{ width: `${failPct}%`, left: `${pct + runPct}%` }}
				/>
			)}
			<span
				className={styles.progressLabel}
				style={{ color: pct > 50 ? "var(--mac-text-inv)" : "var(--mac-text)" }}
			>
		...
		<Panel
			...
			actions={
				<div className={styles.actions}>
					<StatusText status={workflowStatusToRagStatus(workflow.Status)} icon>
						{workflow.Status}
					</StatusText>
					<IconButton label="Back to workflow list" onClick={onBack}>
						← Back
					</IconButton>
				</div>
			}
					...
					items={[
						...
						...(runningCount > 0
							? [
									{
										key: "Running",
										value: <StatusText status="running">{runningCount}</StatusText>,
									},
								]
							: []),
						...(pendingCount > 0
							? [
									{
										key: "Pending",
										value: <StatusText status="pending">{pendingCount}</StatusText>,
									},
								]
							: []),
						...(failedCount > 0
							? [{ key: "Failed", value: <StatusText status="failed">{failedCount}</StatusText> }]
							: []),
					]}
				/>
				<span className={styles.spacer} />
				{isRunning && <Button onClick={onCancel}>Cancel</Button>}
				{workflow.Status === "succeeded" && sourceId && (
					<Button onClick={() => onNavigateToCorpus(sourceId, strategyId)}>View in Corpus →</Button>
				)}
			</div>
		</Panel>
	);
