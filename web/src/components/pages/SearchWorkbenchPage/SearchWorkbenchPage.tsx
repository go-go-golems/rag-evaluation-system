...
export const SearchWorkbenchPage: React.FC = () => {
	...
	const coveragePct = coverage
		? coverage.total_chunks > 0
			? Math.round((coverage.embedded_chunks / coverage.total_chunks) * 100)
			: 0
		: null;
	...
	const openSelectedResultInCorpus = useCallback(() => {
		...
		window.dispatchEvent(
			new CustomEvent("rag:navigate-to-chunk", {
				detail: {
					documentId: selectedResult.document_id,
					chunkId: selectedResult.chunk_id,
					sourceId: selectedResult.source_id,
				},
			}),
		);
	}, [selectedResult]);
			...
			<div className={styles.inspectorRail}>
				{selectedResult ? (
					...
					<Panel title="Inspector" className={styles.emptyInspector}>
						<Caption className={styles.emptyInspectorBody}>Click a result to inspect.</Caption>
					</Panel>
				)}
			</div>
		</DashboardGrid>
